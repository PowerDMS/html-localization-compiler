...
type XliffGenerator() = 
    ...
    let tagToXmlElement (tag : LocalizationTag) =
        ...
        idAttribute.Value <- BitConverter.ToString(sha256Hasher.ComputeHash(System.Text.Encoding.UTF8.GetBytes(flatText + notesText.InnerText)))
    ...
    member x.Generate originalFileName (localizationTags : LocalizationTag seq) =
        ...
        localizationTags |> Seq.iter tagToXmlElement
